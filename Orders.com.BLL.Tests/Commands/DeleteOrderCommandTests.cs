...
namespace Orders.com.BLL.Tests.Commands
{
    ...
    public class DeleteOrderCommandTests
    {
        ...
        [TestMethod]
        public void Execution_should_fail_when_order_is_associated_with_items_that_have_been_shipped_2()
        {
            ...
            orderItemRepo.Clear();
            orderItemRepo.Insert(orderItemRepo.Insert(new OrderItem() { OrderID = order.ID, OrderStatusID = OrderStatusConstants.PENDING_STATUS }));
        ...
        public void Execution_deletes_order_and_associated_order_items()
        {
            ...
            orderDataProxy.Setup(p => p.Delete(1)).Callback((long id) => orders.Remove(id));
            var orderItemDataProxy = new Mock<IOrderItemDataProxy>();
            orderItemDataProxy.Setup(p => p.GetByOrder(It.IsAny<long>()))
        ...
        [TestMethod]
        public void Execution_deletes_order_and_associated_order_items_2()
        {
            var order = new Order() { OrderDate = DateTime.Now };
            var orderRepo = new OrderRepository(Mock.Of<ICustomerDataProxy>(), Mock.Of<IOrderItemDataProxy>());
            orderRepo.Clear();
